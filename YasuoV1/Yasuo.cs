...
namespace YasuoV1
{
    class Yasuo
    {
        ...
        public static void doCombo(Obj_AI_Hero target)
        {
            ...
            if (YasuoS.Config.Item("smartR").GetValue<bool>() && R.IsReady())
                useRSmart();
            if (YasuoS.Config.Item("smartW").GetValue<bool>())
                putWallBehind(target);
            if (YasuoS.Config.Item("useEWall").GetValue<bool>())
                eBehindWall(target);
        ...
        {
            if (!Q.IsReady() || isQEmpovered() || !YasuoS.Config.Item("fleeStack").GetValue<bool>())//fleeStack
                return;
        ...
        public static void doLastHit(Obj_AI_Hero target)
        {
            ...
            foreach (var minion in minions.Where(minion => minion.IsValidTarget(Q.Range)))
            {
                if (Player.Distance(minion) < Orbwalking.GetRealAutoAttackRange(minion) && minion.Health < Player.GetAutoAttackDamage(minion))
                    return;
                if (YasuoS.Config.Item("useElh").GetValue<bool>() && minion.Health < Player.GetSpellDamage(minion, E.Slot))
                    useENormal(minion);

                if (YasuoS.Config.Item("useQlh").GetValue<bool>() && !isQEmpovered() && minion.Health < Player.GetSpellDamage(minion, Q.Slot))
        ...
        public static void doLaneClear(Obj_AI_Hero target)
        {
            ...
            if (YasuoS.Config.Item("useElc").GetValue<bool>() && E.IsReady())
            ...
            if (Q.IsReady() && YasuoS.Config.Item("useQlc").GetValue<bool>())
            {
                if (isQEmpovered() && !(target != null && Player.Distance(target) < 1050))
                {
                    if (canCastFarQ())
                    {
                        ...
                        if (farm.MinionsHit >= YasuoS.Config.Item("useEmpQHit").GetValue<Slider>().Value)
                        {
                    ...
                    {
                        if (minions.Count(min => min.IsValid && !min.IsDead && min.Distance(getDashEndPos()) < QCir.Range) >= YasuoS.Config.Item("useEmpQHit").GetValue<Slider>().Value)
                        {
                            QCir.Cast(Player.Position, false);
        ...
        {
            if (!inTowerRange(Player.ServerPosition.To2D()) || YasuoS.Config.Item("harassTower").GetValue<bool>())
                useQSmart(target);
        }
        ...
        {
            //  if (!YasuoS.Config.Item("djTur").GetValue<bool>())
            //      return false;
        ...
        public static IsSafeResult isSafePoint(Vector2 point)
        {
            ...
            foreach (var skillshot in YasuoS.DetectedSkillshots)
            {
        ...
        public static void useWSmart(Skillshot skillShot)
        {
            ...
            if (skillShot.IsAboutToHit(250, Player))
            {
                ...
                //If enabled
                if (!YasuoS.EvadeSpellEnabled(sd.MenuItemName))
                    return;

                //if only dangerous
                if (YasuoS.Config.Item("wwDanger").GetValue<bool>() &&
                    !YasuoS.skillShotIsDangerous(sd.MenuItemName))
                    return;
                ...
                if (procHp < YasuoS.Config.Item("wwDmg").GetValue<Slider>().Value && Player.Health - spellDamage > 0)
                    return;
                        ...
                        (enemHero.CombatType == GameObjectCombatType.Ranged && dmg > Player.MaxHealth / 8))
                    {
                        if (missleWillHit(missle))
                        {
                            ...
                            if (Player.Distance(missle.Position) < 420)
                            {
                                ...
                                {
                                    YasuoS.lastSpell = missle.SData.Name;
        ...
        public static void useRSmart()
        {
            ...
            foreach (Obj_AI_Hero enem in enemInAir)
            {
                ...
                if (aroundAir >= YasuoS.Config.Item("useRHit").GetValue<Slider>().Value && timeToLand < 0.4f)
                    R.Cast(enem);
            }
        }
